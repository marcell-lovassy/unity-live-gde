...
namespace LiveGameDataEditor.Editor
{
    ...
    public sealed class GameDataColumnDefinition
    {
        ...
        /// </summary>
        public static List<GameDataColumnDefinition> FromType<T>()
        {
        ...
        private static GameDataColumnDefinition BuildDef(FieldInfo f)
        {
            ...
            var label = headerAttr?.Label ?? f.Name;
        ...
        public object ParseListField(string text)
        {
            ...
            {
                var ints = parts.Select(p => int.TryParse(p, out var v) ? v : 0).ToList();
            ...
            {
                var floats = parts.Select(p => float.TryParse(p, out var v) ? v : 0f).ToList();
