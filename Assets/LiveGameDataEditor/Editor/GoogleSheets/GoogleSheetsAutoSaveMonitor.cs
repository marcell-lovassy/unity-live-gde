    ...
    public sealed class GoogleSheetsAutoSaveMonitor : AssetModificationProcessor
    {
        ...
        private static string[] OnWillSaveAssets(string[] paths)
        {
            foreach (var path in paths)
            {
                ...
                if (!_registrations.TryGetValue(guid, out var reg)) continue;
        ...
        {
            if (string.IsNullOrEmpty(containerGuid)) return;
        ...
        {
            if (!string.IsNullOrEmpty(containerGuid)) _registrations.Remove(containerGuid);
        }
    }
}